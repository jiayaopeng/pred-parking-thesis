...
def canopy(X, T1, T2, distance_metric="euclidean", filemap=None):
    ...
    cp_gps_clusters = [
        [key for key, val in canopies.items() if i in val["points"]]
...
def db_scan(
    ...
    db = DBSCAN(
        eps=eps, min_samples=min_samples, algorithm=algorithm, metric=metric
    ).fit(data)
    ...
    print(
        message.format(
            len(data),
            db_num_clusters,
            100 * (1 - float(db_num_clusters) / len(data)),
...
def kmeans(data, n_clusters, random_state=42):
    ...
    print(
        message.format(
            len(data),
            kmeans_num_clusters,
            100 * (1 - float(kmeans_num_clusters) / len(data)),
            time.time() - start_time,
        )
    )
    print(
...
def agg_clustering(data, n_clusters, linkage="ward", compute_distances=True):
    start_time = time.time()

    agg = AgglomerativeClustering(
        n_clusters=n_clusters, compute_distances=compute_distances
    ).fit(data)
    ...
    print(
        message.format(
            len(data),
            agg_num_clusters,
            100 * (1 - float(agg_num_clusters) / len(data)),
            time.time() - start_time,
        )
    )
    print(
