...
def random_forest(X, y, X_test):
    rf = RandomForestClassifier(max_depth=2, random_state=42, class_weight='balanced').fit(X, y)
...
def catboost(X, y, cat_feat, X_test, iterations=1000):
    cb = CatBoostClassifier(iterations=iterations, auto_class_weights='Balanced').fit(X,
                                                                                      y,
