...
def train_all_data(X: pd.DataFrame, X_catboost: pd.DataFrame, cat_feat: list, y: pd.DataFrame):
    """
    ...
    X_train_scaled, X_valid_scaled, X_test_scaled = normalize_data(X_train=X_train,
                                                                                         X_valid=X_valid,
                                                                                         X_test=X_test)
    ## For catboost create train_val, test
    X_train_cat, y_train_cat, X_valid_cat, y_valid_cat, X_test_cat, y_test_cat\
...
def train_different_areas(X_different_areas: pd.DataFrame, X_different_areas_catboost: pd.DataFrame,
                          cat_feat: list, y_different_areas: pd.DataFrame):
    """
    ...
    for area_split in all_area_combinations:
        ...
        X_source_cat, X_target_cat = create_source_target_data(area_split, X_different_areas_catboost)
        ...
        lr_model_areas, lr_preds_areas = bm.logistic_regression(X_source_areas, y_source, X_target_areas)
        lr_result_areas = evaluate_baseline(lr_preds_areas, y_target, "Logistic Regression")
        # random forest
        rf_model_areas, rf_preds_areas = bm.random_forest(X_source_areas, y_source, X_target_areas)
        rf_result_areas = evaluate_baseline(rf_preds_areas, y_target, "Random Forest")

        # catboost classifier, no scaling is needed
        cb_model_areas, cb_preds_areas = bm.catboost(X=X_source_cat,
                                                     y=y_source_cat,
                                                     cat_feat=cat_feat,
                                                     X_test=X_target_cat)
    ...
    df_result_10_areas = pd.DataFrame.from_dict({(i, j): result_areas[i][j]
...
def train_best_model(X_different_areas_catboost_100: pd.DataFrame,
                     cat_feat: list, 
    ...
    for area_split in all_area_combinations:
        ...
        # create data for catboost, without the encoding of categorical feature
        X_source_cat, X_target_cat = create_source_target_data(area_split, X_different_areas_catboost_100)
    ...
    df_result_9_areas = pd.DataFrame.from_dict({(i, j): result_areas[i][j]
                                                 for i in result_areas.keys()
                                                 for j in result_areas[i].keys()},
                                                orient='index')
