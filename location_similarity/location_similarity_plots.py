    ...
    if not cluster_label:
        data.apply(
            lambda row: folium.CircleMarker(
                ...
                weight=2,
                fill=True,
                fill_color=colors[int(
                    data.loc[row.name, study_area]) % len(colors)],
        ...
        data[(data[cluster_label] != -1) & (data[cluster_label] < 10)].apply(
            lambda row: folium.CircleMarker(
                ...
                weight=2,
                fill=True,
                fill_color=colors[data.loc[row.name,
                                           cluster_label] % len(colors)],
...
def plot_highway(data: pd.DataFrame):
    ...
    df_street_highway_capa_groupped = df_street_highway_capa\
        .groupby(['highway'])\
        .agg({'street_id': 'count', 'capacity': 'sum'})\
        .reset_index()\
        .rename(columns={'street_id': 'no. of streets', 'capacity': 'all_capacity_total'})

    df_street_highway_capa_groupped['mean_capacity'] = df_street_highway_capa_groupped['all_capacity_total'] / \
        df_street_highway_capa_groupped['no. of streets']

    plt.figure(figsize=(15, 10))
    plt.bar(df_street_highway_capa_groupped['highway'],
            df_street_highway_capa_groupped['no. of streets'])
    plt.show()
