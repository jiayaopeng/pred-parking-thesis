...
def create_cluster_dict(cluster_col: list, feature_col: list, target_col: list, data: pd.DataFrame) -> dict:
    """
    ...
    for cluster in np.unique(data[cluster_col]): # for each clustering label of that clustering algorithm
        # if in that cluster dataframe, the availability is either only 0 or 1, then remove that cluster from the dataset
        if len(data[(data[cluster_col] == cluster).values][target_col]['availability'].unique()) == 1:
            continue
...
def train_model_on_cluster(
    ...
    for cluster, cluster_df in cluster_dict.items():
        ...
        model.fit(cluster_df[feature_col],
        ...
        matthew_on_train[cluster] = matthews_corrcoef(y_true=y_true_on_train, y_pred=pred_on_train)
...
def calculate_distance(dict_train: dict, dict_test: dict, feature_col: list, cluster_col: list, target_col,
                       time_dependent_col: list) -> dict:
    """
    ...
    for test_cluster, test_value in dict_test.items():
        ...
        for train_cluster, train_value in dict_train.items():
            ...
            distance = KLdivergence(
                ...
                # unique street id
                test_value[[col for col in feature_col if col not in features_excluded_for_kl]].drop_duplicates(),
                train_value[[col for col in feature_col if col not in features_excluded_for_kl]].drop_duplicates()
            )
...
def evaluate_result(
    ...
    matthews_dict = {}
    for (test_cluster_no1, pred), (test_cluster_no2, y_true) in zip(preds_dict.items(), y_trues_dict.items()):
        if test_cluster_no1 == test_cluster_no2:  # do a test
            matthews_dict[test_cluster_no1] = matthews_corrcoef(y_true=y_true, y_pred=pred)
...
def evaluate_valid_result(
    ...
    for cluster_train_no, cluster_df in valid_data.items():
        ...
        preds_dict[cluster_train_no] = model_dict[cluster_train_no].predict(cluster_df[feature_col])
        # get the y_trues
        y_trues_dict[cluster_train_no] = cluster_df[target_col].values.reshape(-1)

        # combine the above as one dictionary
        valid_matthews = {}
        for (cluster_train_no1, pred), (cluster_train_no2, y_true) in zip(preds_dict.items(), y_trues_dict.items()):
            if cluster_train_no1 == cluster_train_no2:
                valid_matthews[cluster_train_no1] = matthews_corrcoef(y_true=y_true, y_pred=pred)
...
def create_cluster_dictionary(
    ...
    encoded_data_test = encoder.transform(
        X=data_test[feature_col],
        y=data_test[target_col]
    )

    # normalize the train and test data's numerical features, take the whole features
    scaler = StandardScaler()
    scaled_data_train = encoded_data_train.copy(deep=True) # copy the data so we do not change the original
    scaled_data_train[[x for x in feature_col if x not in cat_features]] = scaler.fit_transform(
    ...
    scaled_data_test[[x for x in feature_col if x not in cat_features]] = scaler.transform(
    ...
    train_dict = create_cluster_dict(
        ...
        feature_col,
        target_col,
        scaled_merged_data_train
    )
    test_dict = create_cluster_dict(
        cluster_col,
        feature_col,
        target_col,
        scaled_merged_data_test
    )
...
def cluster_train_evaluate_result(
    ...
    train_clusters_dict, test_clusters_dict = create_cluster_dictionary(cluster_col, feature_col, target_col,
    ...
    closest_train_cluster_data_sizes = {} #test cluster number as key, train cluster data size as value
    ...
    for train_cluster_no, train_cluster_df in closest_clusters.items():
        train_cluster_df_80, valid_cluster_df_20 = train_test_split(train_cluster_df, test_size=0.2, random_state=42)
    ...
    model_dict_train_80, matthew_train_80, _ = train_model_on_cluster(
        closest_clusters_train_80, feature_col, target_col, cat_features, iterations=iterations)
    ...
    model_dict, matthews_per_train_cluster, feat_importance_per_train_cluster = train_model_on_cluster(
        closest_clusters, feature_col, target_col, cat_features, iterations=iterations)

    # evaluate the result of model transfer
    scores, matthews_per_test_cluster = evaluate_result(distance_map, model_dict,
                                                        test_clusters_dict, feature_col, target_col)
    ...
    for test_cluster_no, train_cluster_no in distance_map.items():
        feat_importance_dict[test_cluster_no] = feat_importance_per_train_cluster[distance_map[test_cluster_no]]
    ...
    for test_cluster_no, matthew in matthews_per_test_cluster.items():
        matthew_train_size_correlation[test_cluster_no] = {
            ...
            'train_cluster_size': closest_train_cluster_data_sizes[distance_map[test_cluster_no]]
        }
    ...
    for test_cluster_no, matthew in matthews_per_test_cluster.items():
        matthew_train_test[test_cluster_no] = {
            'train_cluster_matthew': matthews_per_train_cluster[distance_map[test_cluster_no]],
            'test_cluster_matthew': matthews_per_test_cluster[test_cluster_no]
        }

    # Evaluate overfiting(within training clusters overfitting)
    matthew_valid_20 = evaluate_valid_result(model_dict_train_80, closest_clusters_val_20, feature_col, target_col)
    # put the two dictionary together
    matthew_train_val = {}
    for (cluster_train_no1, matthew_train), (cluster_train_no2, matthew_val) in zip(matthew_train_80.items(),
                                                                                    matthew_valid_20.items()):
        if cluster_train_no1 == cluster_train_no2:
    ...
    return scores, matthew_train_size_correlation, overfit_final_dict, feat_importance_dict


def train_evaluate_all_approaches(cluster_cols: list, feature_col: list, target_col: list,
                                  df_train_clusters: pd.DataFrame, df_test_clusters: pd.DataFrame,
    ...
    result_matthew_size_corr = {}  # save to see the correlation between training data size and matthew
    ...
    for cluster_col in cluster_cols:
        ...
        if (cluster_col == ['db_cluster_label_gps']) or (cluster_col == ['db_cluster_label_sim']):
            # if the cluster label is of DB scan, then for the model, remove where cluster label = -1 from train and
            # test data set
            train_input = df_train_clusters[(df_train_clusters.db_cluster_label_gps != -1)]
            test_input = df_test_clusters[(df_test_clusters.db_cluster_label_gps != -1)]

        scores, matthew_train_size_corr, matthew_overfit, feature_importance = cluster_train_evaluate_result(
            cluster_col,
            feature_col,
            target_col,
            train_input,
